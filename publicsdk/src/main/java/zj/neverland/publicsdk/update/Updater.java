        ...
        if (downloadId != -1L) {
            ...
            switch (status) {
                ...
                case DownloadManager.STATUS_SUCCESSFUL:
                    ...
                    String path = fdm.getDownloadPath(updaterConfig.getContext(),downloadId);
                    if (uri != null) {
                        uri = Uri.parse(path);
                        //本地的版本大于当前程序的版本直接安装
...
//                    startDownload(updaterConfig);
                    Toast.makeText(updaterConfig.getContext(),"下载失败，请稍后再试！",Toast.LENGTH_SHORT).show();
                    break;
    ...
    private void startDownload(UpdaterConfig updaterConfig) {
        Toast.makeText(updaterConfig.getContext(),"后台正在进行更新....",Toast.LENGTH_SHORT).show();
