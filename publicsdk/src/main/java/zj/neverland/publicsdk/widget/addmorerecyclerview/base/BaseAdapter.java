package zj.neverland.publicsdk.widget.addmorerecyclerview.base;
...
import cn.cefoc.R;
import cn.cefoc.data.NetConfig;
import cn.cefoc.widget.addmorerecyclerview.Util;
import cn.cefoc.widget.addmorerecyclerview.ViewHolder;
import cn.cefoc.widget.addmorerecyclerview.interfaces.OnLoadMoreListener;

/**
 * Time: 2017年4月25日17:55:19
 */
...
//    private boolean isAutoLoadMore = true;//是否自动加载，当数据不满一屏幕会自动加载
    private boolean isLoadingMore = false; //是否正在自动加载
        ...
        recyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
            ...
            public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
                ...
                if (newState == RecyclerView.SCROLL_STATE_IDLE) {
                    if (!isLoadingMore && findLastVisibleItemPosition(layoutManager) + 1 == getItemCount()) {
                        scrollLoadMore();
                    }
                }
            }
    ...
    private void scrollLoadMore() {
        if (mFooterLayout.getChildAt(0) == mLoadingView) {
            if (mLoadMoreListener != null && !isLoadingMore) {
                isLoadingMore = true;
    ...
    public void isAutoLoadMore(boolean isAutoLoadMore) {
        this.isLoadingMore = isAutoLoadMore;
        if(mDatas!=null)
            mDatas.clear();
    }
    ...
    public void setNewData(List<T> datas) {
        ...
        if (datas != null) {
            ...
            notifyDataSetChanged();
            if (datas.size()< NetConfig.PAGE_COUNT){
    ...
    public void setBeginLoadingView(int id){
        removeFooterView();
        setLoadingView(id);
    }
